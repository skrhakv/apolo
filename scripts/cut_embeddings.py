...
#
# For each residue in PDB structure, find out if it is observed or not, get its one-letter code and find its embedding in the embedding numpy array:
...
FILE = 'test.csv'
INPUT_FILE = f'/home/skrhakv/apolo/scripts/test-data/input/csv/{FILE}'
OUTPUT_FILE = f'/home/skrhakv/apolo/scripts/test-data/output/{FILE}'
EMBEDDINGS_INPUT_DIR = '/home/skrhakv/apolo/scripts/test-data/input/embedding'
EMBEDDINGS_OUTPUT_DIR = '/home/skrhakv/apolo/scripts/test-data/output'
HEADER = {
        ...
        for row in reader:
            ...
            for residue in residues_mapping:
                ...
                if str(residue['author_residue_number']) in chain_annotations:
                    #
                    # this produced unnecessary errors due to mismatches in PDB so fuck this :)
                    #
